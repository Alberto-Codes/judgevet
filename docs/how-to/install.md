...
Published judgevet 0.4.1 provides the library, CLI and `judgevet-mcp` command.
...
The published 0.4.1 wheel includes `judgevet/py.typed`. Independent base
installs resolve under their virtual environments' site-packages without the
MCP runtime. See the [published artifact proof](https://github.com/Alberto-Codes/judgevet/issues/124#issuecomment-5777743633).
...
```bash
direnv exec /absolute/path/to/project uvx --from 'judgevet[mcp]==0.4.1' judgevet-mcp
```

This command pins judgevet to version 0.4.1. To update, replace `0.4.1` with a
...
command = "direnv"
args = ["exec", "/absolute/path/to/project", "uvx", "--from", "judgevet[mcp]==0.4.1", "judgevet-mcp"]
```
...
The [published artifact proof](https://github.com/Alberto-Codes/judgevet/issues/124#issuecomment-5777743633)
records byte equality and live base/MCP checks on the actual PyPI download.
The [published launcher proof](https://github.com/Alberto-Codes/judgevet/issues/124#issuecomment-5777743633)

...
```bash
direnv exec /absolute/path/to/project uvx --from 'judgevet[mcp]==0.8.0' judgevet-mcp
```

This command pins judgevet to version 0.8.0. To update, replace `0.8.0` with a
...
command = "direnv"
args = ["exec", "/absolute/path/to/project", "uvx", "--from", "judgevet[mcp]==0.8.0", "judgevet-mcp"]
```

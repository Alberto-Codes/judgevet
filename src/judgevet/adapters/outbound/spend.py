"""Bound the attempts and settled input tokens a caller lets the adapters spend.

A SpendCap is opt-in. An adapter without one sends every attempt its retry
policy permits. An adapter with one claims a slot before each physical
attempt, including every retry, and refuses with JevBudgetExceededError when
a limit is already reached. Nothing is sent on a refusal.
